...
END;
	...
	protected function getPageHeader() {
		...
		if ( $this->subcategory ) {
			...
			$header .= "\n" . '				<span class="drilldown-header-value">' . $subcat_string . '</span> <a href="' . $remove_filter_url . '" title="' . wfMessage( 'sd_browsedata_removesubcategoryfilter' )->text() . '"><img src="' . $wgScriptPath . '/extensions/SemanticDrilldown/skins/filter-x.png" /></a> ';
		}
		foreach ( $this->applied_filters as $i => $af ) {
			...
			foreach ( $af->values as $j => $fv ) {
				...
				$header .= "\n	" . '				<span class="drilldown-header-value">' . $filter_text . '</span> <a href="' . $remove_filter_url . '" title="' . wfMessage( 'sd_browsedata_removefilter' )->text() . '"><img src="' . $wgScriptPath . '/extensions/SemanticDrilldown/skins/filter-x.png" /></a>';
			}
			...
			if ( $af->search_terms != null ) {
				foreach ( $af->search_terms as $j => $search_term ) {
					...
					$header .= "\n\t" . '<span class="drilldown-header-value">~ \'' . $search_term . '\'</span> <a href="' . $remove_filter_url . '" title="' . wfMessage( 'sd_browsedata_removefilter' )->text() . '"><img src="' . $wgScriptPath . '/extensions/SemanticDrilldown/skins/filter-x.png" /> </a>';
				}
			} elseif ( $af->lower_date != null || $af->upper_date != null ) {
